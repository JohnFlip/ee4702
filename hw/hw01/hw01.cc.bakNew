...
//oor next_pos = first_ball;
// // pVect first_to_last = last_ball - first_ball;
//  pNorm f_to_l(first_ball,last_ball);
...
World::ball_setup_2()
{
 ...
 pCoor next_pos = first_ball;
  pVect first_to_last = last_ball - first_ball;
  // float ball_dist = first_to_last.mag()/chain_length;
  //pNorm f_to_l(first_ball,last_ball);
   for ( int i=0; i<chain_length; i++ )
    {
      ...
      ball->contact = false;
     // next_pos += pVect(distance_relaxed,0,0);
     //next_pos += f_to_l; //this just adds the normal. While looks uniform, completely ignores last_ball position
      next_pos += first_to_last/chain_length;
    }
...
World::time_step_cpu(double delta_t)
{
  ...
  /// Problem 2
  //
  //  The following pointers should come in handy.
  //
  Ball* const head_ball = &balls[0];
  Ball* const tail_ball = &balls[chain_length-1];

  /// Parts of Problem 2 solution might go here.
  // pNorm h_to_t(head_ball,tail_ball);
  pVect head_to_tail(head_ball->position,tail_ball->position); //vector from head to tail
  pNorm n_head_to_tail = head_to_tail; //(head_ball->position,tail_ball->position);
  //
  ...
  for ( int i=0; i<chain_length; i++ )
    {
      ...
      /// Problem 2
      //
      //  Parts of solution might go here.
      //NOTE:  When I take the dot product of head_to_tail vector and head_to_ball vector, the value is not = 0. The dot product should = 0, since those vectors are supposed to be orthogonal to each other. The issue may arise when calculating p, the position closest to ball along head_to_tail vector.
       if(opt_twirl == true) {
	pVect head_to_ball(head_ball->position,ball->position);
	float d = dot(head_to_ball,head_to_tail); // should give scalar dist to point closest to Ball along head_to_tail
	pCoor p = head_ball->position + d *n_head_to_tail; //point along h to t closest to B
	//pCoor p = head_ball->position + d * (head_to_tail/head_to_tail.mag());//should be same thing as previous p no?

	pVect p_to_ball(p,ball->position);
	pNorm n_p_to_ball = p_to_ball; //(p,ball->position);
	//float test = dot(p_to_ball,head_to_tail); // why is this not = 0?
	//float test = dot(n_p_to_ball,n_head_to_tail); // why is this not = 0??? gives ~ -.999
	//float  test = dot(head_to_tail,p_to_ball);
	/* to test if position p to ball and p to tail vectors dotted to 0; they don't\
	pVect t(p,tail_ball->position);
	float test = dot(p_to_ball,t);
	if(test == 0)
	  ball_setup_2();
	*/

	pVect f(head_to_tail,p_to_ball); //force orthogonal to head_to_tail and p_to_ball
	//pNorm fn(f);//force of 1?
	pNorm fn = f;
	force += fn; //it seems to keep adding force such that the jump rope effect spins faster and faster
      }
