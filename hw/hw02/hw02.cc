...
World::init()
{
  ...
  coords_p = norms_p = NULL;
  buffer_obj_coords =  buffer_obj_norms = 0;
  coords_size = norms_size = 0;
  ...
  for ( int i=1; i<chain_length; i++ )
    {
      ...
      for ( float t=0; t<1.001; t += delta_t )
        {
          ...
          pVect tang_inner = v12- inner_frac * v12_dist * omega * va * sin(theta)
	  ...
	  if ( t > 0 )
	      ...
	      glTexCoord2f(t,cl);
              ...
              glTexCoord2f(t, cl + (1.0/chain_length));
...
World::render_spiral2()
{
  ...
  //only makes one spiral, not an entire helix
  ...
  // {
      ...
      Ball *const ball3 = &balls[i+1];
      Ball *const ball4 = &balls[i+2];
      pCoor pos3 = ball3->position;
      pCoor pos4 = ball4->position;
	   ...
	   if ( t > 0 )
            {
              ...
              coords += p_inner.x; coords += p_inner.y; coords += p_inner.z;
	      norms += norm_inner.x; norms += norm_inner.y; norms  += norm_inner.z;
  	      coords += p_outer.x; coords += p_outer.y; coords += p_outer.z;
	      norms += norm_prev.x; norms += norm_prev.y; norms  += norm_prev.z;
            }
          p_prev = p;  p_outer_prev = p_outer;  norm_prev = norm;
          p_inner_prev = p_inner; norm_inner_prev = norm_inner;
        } 

      coords_size = coords.occ();
      coords_p = coords.take_storage();
      norms_size = norms.occ();
      norms_p = norms.take_storage();
      ...
      //we can generate one spiral correctly, but something goes wrong with the transform when we loop through the j-loop
 
        glMatrixMode(GL_MODELVIEW);
	glPushMatrix();
       //make_transform returns a matrix m;
	//	for(int j=1;j<chain_length;j+=3){
	// Ball *const ball1 = &balls[j-1];
	//  Ball *const ball2 = &balls[j];
	//  Ball *const ball3 = &balls[j+1];
	//  pos1 = ball1->position;
	//   pos2 = ball2->position;
	//  pos3 = ball3->position;
	  ...
	  // pMatrix coords_trans = make_transform(c_f1,c_t1,c_f2,c_t2);
	   pMatrix coords_trans = make_transform(pos1,pos2,pos2,pos3);
       //  glPopMatrix();
       //  glPushMatrix();
       glMultTransposeMatrixf(coords_trans);  
      ...
      glDrawArrays(GL_TRIANGLE_STRIP, 0, coords_size/3);//divide by three since coords stores x,y,z components
      glBindBuffer(GL_ARRAY_BUFFER,0);
...
glPopMatrix();
 ...
 if ( opt_recompute )
    {
      free( coords_p );  coords_p = NULL;  gpu_buffer_stale = true;
    }
