...
World::init()
{
  ...
  eye_location = pCoor(-28.2, 70.4, 69.4);
  eye_direction = pVect(0.76, -0.6, 0.01);
  ...
  light_location = pCoor(-88, 70.0, 31.7);
  ...
  drip_location = pCoor(79, 40.76 ,60); // x = 30, y=20
...

  {
    ...
    const float x2 = platform_xmax;
    const float x1 = platform_xmin;
    const float step_size = ( x2 - x1 ) / step_count;
    const pVect step_hor(step_size,0,0);
    const pVect step_ver(0,step_size*0.35,0);
    pVect diag(step_size,step_size*0.35,0);
    // const pVect step_wid(0,0,20);
    const pVect step_wid(0,0,160);
    //const pCoor step_start(x1,-platform_xrad*0.9,0);
    const pCoor step_start(0,13,0);
    pColor step_color = color_burlywood;//(0.5,0.5,0.5);
    for ( int i=0; i<step_count; i++ )
      {
        const pCoor step_ll = step_start + i *( step_hor + step_ver );
	tile_manager.new_tile(step_ll,diag,step_wid,step_color);
	//tile_manager.new_tile(step_ll,step_ver,step_wid,step_color);
...
World::cb_keyboard()
{
  ...
  const float move_amt = shift? 4.0 : 0.4;
